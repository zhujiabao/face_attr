...
def resnet50(pretrained=False, **kwargs):
    ...
    if pretrained:
        model.load_state_dict(torch.load(model.modelPath), False)
    return model
...
class ResNet(nn.Module):
    ...
    def __init__(self, layers, num_classes=40, model_path=None):
        ...
        #self.fc = nn.Linear(512 * Bottleneck.expansion, num_classes*2)
        for c in range(self.num_classes):
            self.__setattr__('class_%d' %c, ClassBlock(class_num=1, activ='sigmod'))
    ...
    def forward(self, x):
        ...
        pred_label = [self.__getattr__("class_%d" %c)(x) for c in range(self.num_classes)]
